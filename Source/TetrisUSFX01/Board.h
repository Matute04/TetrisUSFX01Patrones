...
public:	
	...
	UPROPERTY()
	ASingletonBloque* BloqueI;
	UPROPERTY()
	APiezaDecorator* PiezaDecorator;
	UPROPERTY()
	APiezaDecoratorIzquierda* PiezaDecoratorIzquierda;
	UPROPERTY()
	APiezaBorde* PiezaBorde;
	UPROPERTY()
	UAdaptadorMovimientoAleatorioCA* AdapterPieza;
	UPROPERTY()
...
private:
	...
	int cont = 0;
	float PosicionZ = 180.0f;
	bool CheckGameOver();											  // función que verifica si el juego ha terminado.

	UPROPERTY(VisibleAnywhere, Category = "Main")
		class APiezaBrillante* PiezaBrillante;

	UPROPERTY(VisibleAnywhere, Category = "Main")
		class APiezaDorada* PiezaDorada;

	UPROPERTY(VisibleAnywhere, Category = "Main")
		class APiezaMetalica* PiezaMetalica;

	UPROPERTY(VisibleAnywhere, Category = "Main")
		class APiezaHielo* PiezaHielo;

	UPROPERTY(VisibleAnywhere, Category = "Main")
		class APiezaPiedra* PiezaPiedra;

	UPROPERTY(VisibleAnywhere, Category = "Main")
		class APiezaMulticolor* PiezaMulticolor;
