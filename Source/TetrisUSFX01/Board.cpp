...
void ABoard::BeginPlay()                            // Función que se ejecuta cuando el juego comienza o cuando se crea el actor
{
    ...
    for (int i = 0; i <= 3; i++)
    {
        FVector Location(0.0, 100.0, PosicionZ);
        FRotator Rotation(0.0, 0.0, 0.0);
        BloqueI = GetWorld()->SpawnActor<ASingletonBloque>(Location, Rotation);
        if (cont < 1)
        {
            GEngine->AddOnScreenDebugMessage(-1, 15.f, FColor::Yellow, FString::Printf(TEXT("Bloque Creado")));
        }
        //PosicionZ = PosicionZ - 50.0f;
        cont = cont + 1;
    }
    FVector Location2(0.0f, -100.0f, 180.0f);
    FRotator Rotation2(0.0f, 0.0f, 0.0f);
    PiezaBorde = GetWorld()->SpawnActor<APiezaBorde>(Location2, Rotation2);
    PiezaDecorator = GetWorld()->SpawnActor<APiezaDecorator>(APiezaDecorator::StaticClass());
    PiezaDecorator->SetPiezaRotatoria(PiezaBorde);
    PiezaDecoratorIzquierda = GetWorld()->SpawnActor<APiezaDecoratorIzquierda>(APiezaDecoratorIzquierda::StaticClass());
    PiezaDecoratorIzquierda->SetPiezaRotatoria(PiezaDecorator);
    PiezaDecoratorIzquierda->Rotar();
}
...
void ABoard::NewPiece()  // Crear una nueva pieza
{
    ...
    PiezaBrillante = GetWorld()->SpawnActor<APiezaBrillante>(APiezaBrillante::StaticClass());
    PiezaMetalica = GetWorld()->SpawnActor<APiezaMetalica>(APiezaMetalica::StaticClass());
    PiezaPiedra = GetWorld()->SpawnActor<APiezaPiedra>(APiezaPiedra::StaticClass());
    PiezaDorada = GetWorld()->SpawnActor<APiezaDorada>(APiezaDorada::StaticClass());
    PiezaHielo = GetWorld()->SpawnActor<APiezaHielo>(APiezaHielo::StaticClass());
    PiezaMulticolor = GetWorld()->SpawnActor<APiezaMulticolor>(APiezaMulticolor::StaticClass());
    int Random = FMath::RandRange(0, 5);
    //int Random = 1;
    switch (Random)
    {
    case 0:
        PiezaDirector->SetConstruirPieza(PiezaBrillante);
        GEngine->AddOnScreenDebugMessage(-1, 4, FColor::Yellow, TEXT("Pieza Brillante"));
        break;
    case 1:
        PiezaDirector->SetConstruirPieza(PiezaMetalica);
        GEngine->AddOnScreenDebugMessage(-1, 4, FColor::Yellow, TEXT("Pieza Metalica"));
        break;
    case 2:
        PiezaDirector->SetConstruirPieza(PiezaPiedra);
        GEngine->AddOnScreenDebugMessage(-1, 4, FColor::Yellow, TEXT("Pieza Piedra"));
        break;
    case 3:
        PiezaDirector->SetConstruirPieza(PiezaDorada);
        GEngine->AddOnScreenDebugMessage(-1, 4, FColor::Yellow, TEXT("Pieza Dorada"));
        break;
    case 4:
        PiezaDirector->SetConstruirPieza(PiezaHielo);
    ...
    if (Random == 3)
    {
        GEngine->AddOnScreenDebugMessage(-1, 3, FColor::Green, TEXT("Se genero la pieza loca DORADA"));
